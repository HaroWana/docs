        ...
        context.update(
            {
                ...
                "button_link": f"{domain}/api/v1.0/documents/{self.id}/{context.get('button_link')}", 
    ...
    def send_access_request_email(self, requester, language=None):
        ...
        with override(language):
            context = {
                ...
                ).format(name=requester_name, email=requester_email, path=self.path, title=self.title),
                "button_link": f"grant-access/{requester.id}",
        ...
        admin_emails = (
            ...
            .filter(document=self, role__in=[RoleChoices.ADMIN, RoleChoices.OWNER])
            .select_related("user")
